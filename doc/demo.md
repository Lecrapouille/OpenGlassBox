...
Files are handled through File → New city, Open ruleset (`.ogs`), Open city, and Save city (`.ogc`). The Script panel edits the open `.ogs`; **Apply** reparses it, keeping the city as long as every type it still uses is defined by the new ruleset.

A save records the fingerprint of the ruleset it was written against and refuses to open against one that changed since, which is what stops a city from being rebuilt out of types that mean something else. Editing a script therefore breaks every save beside it at once, so Apply stamps them with the new fingerprint and the panel reports how many. When one goes stale by another route, the panel names it and **Update them** does the same thing; only the header line is rewritten, and the types a save names are still required to exist. **Checksum details** holds the three fingerprints for copying, and File → **Open saves with a stale checksum** waives the check outright.
