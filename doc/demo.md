...
- **Layers** also carries the layer list, since choosing which layer to paint and which layer to look at is the same decision. Each row toggles the visibility of a layer, sets its opacity, and picks how it is drawn: filled cells, contours, or numeric values. Clicking a name makes it the main layer; Alt+clicking shows it alone. A simulation opens with one layer shown, because half a dozen heatmaps stacked on the same cells cannot be read.
