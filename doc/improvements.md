...
- The original project uses C# for Unity. This is C++17, with no game engine underneath.
- The simulation is a library that knows nothing about drawing; the demo is a separate program on top of it.
- Confusing names from the GDC talk were renamed: `Box` → `City`, `Point`/`Segment` → `Node`/`Segment`, `ResourceBinCollection` → `Resources`, `SimulationDefinitionLoader` → `Ruleset`. The term `Building` was kept (it means building). New classes include `Zone` (zone).
...
- **Zones exist.** The original declared the `Zone` class, also called `Zone`, and never implemented it. Here `Zone` and `ZoneRule` are real, with `spawn`, `upgrade`, `destroy`, and `count` commands. A zone is a rectangle of cells, and a building is not a graph node, so the two stay independent.
- **A building is no longer a crossroads.** A `Building` used to be forced onto a `Node` of a `Path`, which made the graph explode. It now has its own position plus an optional anchor, either on a `Node` or at an offset along a `Segment`. That anchor is not decoration: `spawn ... at nearestSegment` only grows a building on a cell a road runs through or fronts, because a building nobody can drive to could neither send nor receive an `Agent`. An `Agent` leaving a building anchored along a segment drives out by the end its destination lies behind, which is not always the near one, and it only knocks at a door it has actually driven to.
- **Routing is pluggable and traffic-aware.** The original walked the graph with a dynamic A\* inside `Path::FindNextPoint`. Routing now goes through the `IRouter` interface, and the shipped implementation minimises BPR travel times rather than distances. The flow of each `Segment` is an exponential moving average, **not** the MSA solver of CiudadSim; see [how traffic is modelled](traffic.md).
