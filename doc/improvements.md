...
- Confusing names from the GDC talk were renamed: `Box` → `City`, `Point`/`Segment` → `Node`/`Segment`, `ResourceBinCollection` → `Resources`, `SimulationDefinitionLoader` → `Ruleset`. The term `Unit` was kept (it means building). New classes include `Zone` (zone).
...
- **A building is no longer a crossroads.** A `Unit` used to be forced onto a `Node` of a `Path`, which made the graph explode. It now has its own position plus an optional anchor, either on a `Node` or at an offset along a `Segment`. That anchor is not decoration: `spawn ... at nearestSegment` only grows a building on a cell a road runs through or fronts, because a building nobody can drive to could neither send nor receive an `Agent`. An `Agent` leaving a building anchored along a segment drives out by the end its destination lies behind, which is not always the near one, and it only knocks at a door it has actually driven to.
