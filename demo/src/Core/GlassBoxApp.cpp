...
void appendSearchPaths(std::vector<std::string>& paths, std::string const& spec)
{
    if (spec.empty())
        return;
...
std::vector<std::string> buildDataPaths(std::string const& commandLinePath)
{
    std::vector<std::string> paths;

    if (!commandLinePath.empty())
        appendSearchPaths(paths, commandLinePath);

    char const* const env = ::getenv(ENV_DATA_PATH);
    if ((env != nullptr) && (env[0] != '\0'))
        appendSearchPaths(paths, env);

    appendSearchPaths(paths, "demo/data/");
    appendSearchPaths(paths, "data/");
    paths.push_back("./");
    ...
    {
#if defined(__APPLE__)
        paths.push_back(exeDir + "../Resources/");
#endif
        paths.push_back(exeDir + "data/");
        paths.push_back(exeDir + "../demo/data/");
        paths.push_back(exeDir);
    }

    appendSearchPaths(paths, project::info::paths::data);

    return paths;
}

std::pair<std::string, bool> findDataFile(std::vector<std::string> const& paths,
                                          std::string const& filename)
{
    if (fileExists(filename))
        return {filename, true};

    for (std::string const& directory: paths)
    {
        std::string const file = directory + filename;
        if (fileExists(file))
            return {file, true};
    }
...
std::string dataPathsToString(std::vector<std::string> const& paths)
{
    std::string joined;
...
{
    auto const found = findDataFile(m_data_paths, filename);
    if (!found.second)
    {
        m_script_error = "Cannot find '" + filename + "' in the data path " +
                         dataPathsToString(m_data_paths);
        return false;
    }
