...
// ----------------------------------------------------------------------------
void CityViewer::frameAll(Simulation const& simulation)
{
    ...
    for (auto const& [_, cityPtr] : simulation.getCities())
    {
        ...
        // The grid of the city always bounds its layers, and the nodes may
        // stick out of it, so take the union of both.
        float const side = city.getCellSize();
        minX = std::min(minX, city.getPosition().x);
        minY = std::min(minY, city.getPosition().y);
        maxX = std::max(
            maxX, city.getPosition().x + float(city.getRegion().sizeU) * side);
        maxY = std::max(
            maxY, city.getPosition().y + float(city.getRegion().sizeV) * side);
        found = true;

        for (auto const& [_, path] : city.getPaths())
        {
                      ...
                      editor::Editor& editor)
{
    ...
    // The layers belong to the world and span every city, so they are drawn
    // once rather than once per city.
...
// ----------------------------------------------------------------------------
Segment* CityViewer::pickSegment(City const& city,
                                 ImVec2 const& world,
                                 float pixels,
                                 float& offset) const
{
    ...
    for (auto const& [_, path] : city.getPaths())
    {
        for (auto const& segment : path->getSegments())
        {
            ...
            float const distance = distanceToSegment(world,
...
// ----------------------------------------------------------------------------
Node* CityViewer::pickNode(City const& city,
                           ImVec2 const& world,
                           float pixels) const
{
    float const tolerance = pixels / std::max(1e-3f, m_zoom);
...
// ----------------------------------------------------------------------------
Building* CityViewer::pickBuilding(City const& city,
                                   ImVec2 const& world,
                                   float pixels) const
{
    float const tolerance = pixels / std::max(1e-3f, m_zoom);
...
// ----------------------------------------------------------------------------
Agent*
CityViewer::pickAgent(City const& city, ImVec2 const& world, float pixels) const
{
    float const tolerance = pixels / std::max(1e-3f, m_zoom);
    float best = tolerance * tolerance;
    Agent* found = nullptr;
...
// ----------------------------------------------------------------------------
game::Selection CityViewer::pickAt(Simulation const& simulation,
                                   ...
                                   ImVec2 const& screen) const
{
    ...
    if (state.showPaths)
    {
        ...
        for (auto const& [cityName, cityPtr] : simulation.getCities())
        {
            float offset = 0.0f;
            Segment* segment =
                pickSegment(*cityPtr, world, PICK_RADIUS, offset);
            if (segment == nullptr)
                continue;
...
// ----------------------------------------------------------------------------
void CityViewer::pick(Simulation const& simulation,
                      game::DebugState& state,
                      ImVec2 const& screen)
{
...
// ----------------------------------------------------------------------------
void CityViewer::updateHover(Simulation const& simulation,
                             game::DebugState& state,
                             bool hovered)
{
...
// ----------------------------------------------------------------------------
void CityViewer::drawLayers(Simulation const& simulation,
                            game::DebugState const& state)
{
    ...
    int32_t valueRows = 0;
    for (auto const& [_, layerPtr] : simulation.getLayers())
    {
    ...
    for (auto const& [_, layerPtr] : simulation.getLayers())
    {
        ...
        bool const primary =
            (state.primaryLayer == layer.getTypeName().str()) ||
        ...
        layer.forEachBlockInRegion(
            ...
            [&](int32_t u, int32_t v, int32_t cells, uint32_t amount)
            {
                ...
                switch (options.mode)
                {
                    ...
                    {
                        // A non primary layer is drawn thinner so that several
                        // of them stay readable when superimposed.
...
void CityViewer::drawCityFrame(City const& city, game::DebugState const& state)
{
    ...
    ImVec2 const bottomRight = worldToScreen(
        city.getPosition().x + float(city.getRegion().sizeU) * side,
...
// ----------------------------------------------------------------------------
void CityViewer::drawZones(City const& city, game::DebugState const& state)
{
    m_splitter.SetCurrentChannel(m_draw_list, CHANNEL_GRID);
    ...
    for (auto const& zone : city.getZones())
    {
        ...
        if (labels)
        {
            m_draw_list->AddText(ImVec2(p0.x + 4.0f, p0.y + 2.0f),
                                 line,
...
// ----------------------------------------------------------------------------
void CityViewer::drawPaths(City const& city, game::DebugState const& state)
{
    if (!state.showPaths)
        return;
        ...
        for (auto const& segment : path.getSegments())
        {
            ...
            if (state.showTraffic)
            {
                ...
                float const saturation =
                    std::min(1.5f, segment->getSaturation());
...
// ----------------------------------------------------------------------------
void CityViewer::drawBuildings(City const& city, game::DebugState const& state)
{
    if (!state.showBuildings)
        return;
    ...
    for (auto const& building : city.getBuildings())
    {
        ...
        if ((segment != nullptr) && state.showPaths)
        {
            Vector3f const anchor =
                segment->getPositionAt(building->getSegmentOffset());
        ...
        // A square, so that a Building is never confused with a Node or an
        // Agent.
        m_draw_list->AddRectFilled(
...
// ----------------------------------------------------------------------------
void CityViewer::drawAgents(City const& city, game::DebugState const& state)
{
    if (!state.showAgents)
        return;
...
// ----------------------------------------------------------------------------
void CityViewer::drawInspectHover(Simulation const& simulation,
                                  game::DebugState const& state,
                                  editor::Editor const& /*editor*/)
{
    ...
    switch (hover.kind)
    {
        ...
        case game::Selection::Kind::Zone:
        {
            ...
            ImVec2 const p0 =
                worldToScreen(city.cellToWorld({ footprint.u0, footprint.v0 }));
...
// ----------------------------------------------------------------------------
void CityViewer::drawSelectionOverlay(Simulation const& simulation,
                                      game::DebugState const& state,
                                      editor::Editor const& editor)
{
    m_splitter.SetCurrentChannel(m_draw_list, CHANNEL_OVERLAY);
    ...
    switch (selection.kind)
    {
        case game::Selection::Kind::Building:
        {
            ...
            ImVec2 const position =
                worldToScreen(selection.building->getPosition());
            m_draw_list->AddCircle(
                position, UNIT_RADIUS + 6.0f, highlight, 0, 2.0f);

            if (state.showSelectionRadius)
            {
                // Materialize the disc the rules of this Building read and
                // write on the layers. Seeing it is the quickest way to
                // understand why a Building does not reach the resource next
                // door.
                float const radius =
                    float(selection.building->getLayerRadius()) *
        ...
        case game::Selection::Kind::Cell:
        {
            float const side = city.getCellSize();
            ImVec2 const p0 =
                worldToScreen(city.cellToWorld({ selection.u, selection.v }));
            ImVec2 const p1(p0.x + side * m_zoom, p0.y + side * m_zoom);
            m_draw_list->AddRect(p0, p1, highlight, 0.0f, 0, 2.5f);
            break;
        }
        ...
        case game::Selection::Kind::Segment:
        {
            if (selection.segment == nullptr)
                break;
            ImVec2 const a =
                worldToScreen(selection.segment->getFromPosition());
...
// ----------------------------------------------------------------------------
void CityViewer::drawHoverTooltip(Simulation const& simulation,
                                  game::DebugState const& state,
                                  editor::Editor const& editor)
{
    if (editor.tool() != editor::EditTool::Select)
    ...
    switch (hover.kind)
    {
        case game::Selection::Kind::Building:
            if (hover.building != nullptr)
            {
                ...
                ImGui::Separator();
                std::vector<Resource> const& bin =
                    building.getResources().getAll();
                if (bin.empty())
                {
        ...
        case game::Selection::Kind::Cell:
        {
            ...
            {
                for (auto const& [_, layerPtr] : city.getLayers())
                {
                    Layer const& layer = *layerPtr;
                    ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(
                                           theme::fromScript(layer.getColor())),
...
void CityViewer::drawClockHud(Simulation const& simulation)
{
    ...
    char day[24];
    std::snprintf(day, sizeof(day), "Day %u", clock.getDay());
