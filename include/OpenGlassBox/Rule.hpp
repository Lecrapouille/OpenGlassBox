...
//! of a layer fills in \c map, a rule of a zone fills in \c area. \c city is
//! always set.
//!
...
struct RuleContext
{
    //! \brief The town the rule is running on behalf of. Never null while a
    //! rule is running.
    City* city = nullptr;
    ...
    //! \brief What \c global reads and writes: the resources of the town as a
    //! whole, such as money.
    Resources* globals = nullptr;
    //! \brief Column of the cell the rule acts on. Signed because the grid is
    //! unbounded in the four directions.
    int32_t u = 0;
    //! \brief Row of the cell the rule acts on.
    int32_t v = 0;
...
//!
//! A script writes \c local \c People, \c global \c Money or \c map \c Water,
...
//! context.city = &city;
//! context.unit = &home;
//! context.clock = &simulation.clock();
//! context.locals = &home.resources();
//! context.globals = &city.globals();
//!
//! for (RuleUnit* rule : home.rules())
//! {
//!     // A script may name a rule that does not exist, hence the test.
//!     if ((rule != nullptr) &&
//!         (home.ticks() % rule->periodTicks(ticksPerMinute) == 0u))
//!     {
...
public:
    ...
    struct Trace
    {
        ...
        bool success = false;
        //! \brief Which command refused, as an index into rule->commands(), or
    ...
    //! town, so a pointer per rule would cost memory for something that is off
    ...
    //! when the script counted ticks. See periodTicks().
    ...
    //! when the script gave a duration instead: ask periodTicks().
    //--------------------------------------------------------------------------
    uint32_t rate() const
    {
        return m_rate;
    ...
    //! than while parsing, so that changing SimulationConfig::ticksPerMinute
    ...
    //--------------------------------------------------------------------------
    std::vector<IRuleCommand*> const& commands() const
    {
        return m_commands;
    }
...
//==============================================================================
//! \brief A rule belonging to a layer, run over the cells of a town.
//!
...
public:
    ...
    //! not kept.
    //--------------------------------------------------------------------------
    explicit RuleMap(RuleMapType const& type)
...
public:
    ...
    //! not kept.
    //--------------------------------------------------------------------------
    explicit RuleArea(RuleAreaType const& type)
        : IRule(type.name, type.rate, type.rateMinutes, type.commands)
    {
    }
